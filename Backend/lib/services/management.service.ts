...
const db = (client: DbClient) => (client as any).schema("management");
...
export async function createBudgetRequest(client: DbClient, input: Record<string, unknown>) {
  const { data, error } = await db(client).from("t_budget_request").insert(input).select("*").single();
  return { data: data as TBudgetRequest | null, error };
}
...
export async function createKPIWeekly(client: DbClient, input: Record<string, unknown>) {
  const { data, error } = await db(client).from("t_kpi_weekly").insert(input).select("*").single();
