...
type QcInboundPayload = {
  qc_inbound: TQCInbound | null;
};
...
export default function QcInboundPage() {
  ...
  const [editData, setEditData] = useState<TQCInbound | null>(null);
  ...
  const fetchQcInbound = async () => {
    try {
      ...
      setItems(payload.data.qc_inbound ?? []);
    } catch (error) {
      const message = error instanceof Error ? error.message : "Gagal memuat data QC inbound.";
      alert(message);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Gagal menghapus data QC inbound.";
      alert(message);
    } finally {
      setIsSubmitting(false);
      closeDeleteModal();
    ...
    <div className="p-4 md:p-6 lg:p-8 space-y-4 md:space-y-6 max-w-7xl mx-auto w-full">
      ...
      <section className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="px-4 md:px-6 py-4 border-b border-slate-100">
          ...
          <p className="mt-1 text-xs md:text-sm text-slate-500">Rekap inspeksi bahan baku masuk berdasarkan produksi order.</p>
        </div>
      ...
      <Modal isOpen={isFormModalOpen} onClose={closeFormModal} title="Input Hasil QC Inbound" maxWidth="max-w-lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label htmlFor="status-qc-inbound" className="text-sm font-semibold text-slate-700">Status Hasil QC</label>
            <select
      ...
      <ConfirmDialog
        ...
        title="Hapus Data QC Inbound"
        description="Apakah Anda yakin ingin menghapus data QC inbound ini?"
