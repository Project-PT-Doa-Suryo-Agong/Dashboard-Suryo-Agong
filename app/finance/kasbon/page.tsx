...
export default function FinanceKasbonPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    tipe: "kasbon" as FinanceUtangPiutangTipe,
    coa: "",
  });
  ...
  // Filter COA untuk Kasbon (menggunakan kelompok akun Piutang: 12xx)
  ...
  const handleExportPDF = () => {
    ...
    exportToPDF({
      title: "Laporan Catatan Kasbon",
      headers: ["Karyawan", "Tanggal Awal", "Jatuh Tempo", "Nominal", "Lunas (Kas)", "Overdue", "COA/Akun"],
      ...
      summary: [
        { label: "Total Kasbon", value: `${exportRows.length} catatan` },
      ...
      fileName: "Laporan_Catatan_Kasbon_PT_Doa_Suryo_Agong.pdf",
    });
  };
  ...
  const handleExportExcel = () => {
    ...
    void exportToExcel({
      ...
      moneyColumns: [4],
      columnAlign: { 4: "right" },
      fileName: `Laporan_Catatan_Kasbon_${new Date().toISOString().slice(0, 10)}.xlsx`,
    });
  };
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 space-y-4 max-w-7xl mx-auto w-full">
      <section className="flex flex-col sm:flex-row justify-between gap-3 items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Catatan Kasbon</h1>
          <p className="text-slate-300">
            Kelola catatan kasbon atau pinjaman karyawan dari perusahaan.
          </p>
        </div>
      </section>
      ...
      <section className="flex flex-col sm:flex-row justify-between gap-3 items-center">
        <SearchBar
          ...
          placeholder="Cari karyawan, deskripsi, atau COA..."
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-white font-semibold hover:bg-blue-700 transition"
          >
            <PlusCircle className="h-5 w-5" /> Catat Kasbon Baru
          </button>
        </div>
      </section>
                ...
                filteredItems.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3">
                      <span className="px-2 py-1 text-xs rounded-full font-semibold bg-blue-600 text-white">
                        KASBON
                      </span>
                    </td>
      ...
      <Modal
        ...
        title={editData ? "Edit Catatan Kasbon" : "Catat Kasbon Baru"}
        maxWidth="max-w-2xl"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <input
                placeholder="Detail keperluan kasbon..."
          ...
          <div className="flex flex-col sm:flex-row sm:justify-end gap-3 pt-4 border-t border-slate-200 mt-6">
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="inline-flex items-center justify-center rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 transition disabled:opacity-50"
            >
      ...
      <ConfirmDialog
        ...
        description="Yakin hapus catatan kasbon ini?"
        variant="danger"
      />
    </div>
  );
