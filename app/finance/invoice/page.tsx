...
export default function FinanceInvoicePage() {
  ...
  useEffect(() => { void fetchDependencies(); }, []);
  ...
  const openEditModal = async (item: TInvoice) => {
    ...
    try {
      const res = await apiFetch(`/api/finance/invoice-items?invoice_id=${item.id}`);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const url = editData ? `/api/finance/invoice/${editData.id}` : "/api/finance/invoice";
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                ...
                <tr key={item.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-mono text-xs text-slate-500" title={item.id}>{item.id.substring(0, 8)}...</td>
        ...
        {detailData && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100">
              <div><p className="text-slate-500 text-xs font-semibold uppercase">ID Invoice</p><p className="font-mono mt-1 text-slate-800">{detailData.invoice.id}</p></div>
