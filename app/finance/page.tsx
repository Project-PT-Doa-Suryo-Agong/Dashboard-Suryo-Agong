...
export default function FinanceDashboardPage() {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const cashflowPayload =
          await parseJsonResponse<CashflowListPayload>(cashflowResponse);
        const reimbursePayload =
          await parseJsonResponse<ReimburseListPayload>(reimburseResponse);
        ...
        setLoadError(
          error instanceof Error
            ? error.message
            : "Gagal memuat data dashboard finance.",
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const cashflowPreviewData = useMemo<CashflowPoint[]>(() => {
    ...
    for (let i = 5; i >= 0; i -= 1) {
      ...
      const labelRaw = new Intl.DateTimeFormat("id-ID", {
        month: "short",
      }).format(date);
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 space-y-4 md:space-y-6 lg:space-y-8 max-w-7xl mx-auto w-full">
      <section className="space-y-1 md:space-y-2">
        <h1 className="text-lg md:text-2xl lg:text-3xl font-bold text-slate-100">
          Finance Command Center
        </h1>
        <p className="text-sm md:text-base text-slate-300">
          Monitor arus kas, persetujuan dana, dan ringkasan finansial
          perusahaan.
        </p>
      </section>
        ...
        <article className="bg-white border border-slate-200 shadow-sm rounded-xl p-4 md:p-6">
          <div className="flex items-start justify-between gap-3">
            <div className="space-y-2 min-w-0">
              <p className="text-xs uppercase tracking-wide font-semibold text-slate-500 truncate">
                Total Saldo Perusahaan
              </p>
              <p className="text-base md:text-2xl font-bold text-slate-900 wrap-break-word">
                {formatRupiah(metrics.totalBalance)}
              </p>
            </div>
        ...
        <article className="bg-white border border-slate-200 shadow-sm rounded-xl p-4 md:p-6">
          <div className="flex items-start justify-between gap-3">
            <div className="space-y-2 min-w-0">
              ...
              <p className="text-base md:text-2xl font-bold text-emerald-600 wrap-break-word">
                {formatRupiah(metrics.monthlyIncome)}
              </p>
            </div>
        ...
        <article className="bg-white border border-slate-200 shadow-sm rounded-xl p-4 md:p-6">
          <div className="flex items-start justify-between gap-3">
            <div className="space-y-2 min-w-0">
              ...
              <p className="text-base md:text-2xl font-bold text-red-600 wrap-break-word">
                {formatRupiah(metrics.monthlyExpense)}
              </p>
            </div>
            <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-red-500 text-white">
      ...
      <section className="bg-white border border-slate-200 shadow-sm rounded-xl p-4 md:p-6">
        <div className="mb-3 md:mb-4">
          ...
          <p className="text-xs md:text-sm text-slate-500 mt-1">
            Preview interaktif pemasukan vs pengeluaran 6 bulan terakhir.
          </p>
        </div>
      ...
      <section>
        <div>
          <h1 className="text-lg md:text-xl font-bold text-slate-100 py-4">
            Quick Links
          </h1>
        </div>
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-3 min-w-0">
                <div className="inline-flex items-center rounded-full border border-orange-200 bg-amber-500 px-2.5 py-1 text-[11px] font-semibold text-white">
                  {isLoading
                    ? "Loading..."
                    : `${pendingReimburseCount} Pending`}
                </div>
                ...
                <p className="text-sm text-slate-500 wrap-break-word">
                  Tinjau dan proses pengajuan dana yang menunggu persetujuan.
                </p>
              </div>
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-3 min-w-0">
                ...
                <p className="text-sm text-slate-500 wrap-break-word">
                  Tinjau dan proses payroll karyawan dengan mudah.
                </p>
              </div>
              <ArrowRight className="h-4 w-4 md:h-5 md:w-5 text-slate-400 group-hover:text-slate-600 group-hover:translate-x-1 transition-all shrink-0" />
            </div>
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-3 min-w-0">
                ...
                <p className="text-sm text-slate-500 wrap-break-word">
                  Pantau arus kas masuk dan keluar secara terstruktur per
                  periode.
                </p>
              </div>
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-3 min-w-0">
                ...
                <p className="text-sm text-slate-500 wrap-break-word">
                  Tinjau dan proses chart of accounts dengan mudah.
                </p>
              </div>
              <ArrowRight className="h-4 w-4 md:h-5 md:w-5 text-slate-400 group-hover:text-slate-600 group-hover:translate-x-1 transition-all shrink-0" />
            </div>
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-3 min-w-0">
                ...
                <p className="text-sm text-slate-500 wrap-break-word">
                  Tinjau dan proses jurnal umum dengan mudah.
                </p>
              </div>
              <ArrowRight className="h-4 w-4 md:h-5 md:w-5 text-slate-400 group-hover:text-slate-600 group-hover:translate-x-1 transition-all shrink-0" />
            </div>
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-3 min-w-0">
                <div className="inline-flex items-center rounded-full border border-slate-200 bg-red-600 px-2.5 py-1 text-[11px] font-semibold text-white">
                  Kewajiban
                </div>
                <h2 className="text-base md:text-lg font-bold text-slate-900 wrap-break-word">
                  Catatan Utang
                ...
                <p className="text-sm text-slate-500 wrap-break-word">
                  Tinjau dan kelola catatan hutang usaha perusahaan.
                </p>
              </div>
              <ArrowRight className="h-4 w-4 md:h-5 md:w-5 text-slate-400 group-hover:text-slate-600 group-hover:translate-x-1 transition-all shrink-0" />
            </div>
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-3 min-w-0">
                <div className="inline-flex items-center rounded-full border border-slate-200 bg-green-600 px-2.5 py-1 text-[11px] font-semibold text-white">
                  Tagihan
                </div>
                <h2 className="text-base md:text-lg font-bold text-slate-900 wrap-break-word">
                  Catatan Piutang
                ...
                <p className="text-sm text-slate-500 wrap-break-word">
                  Tinjau dan kelola catatan piutang dan tagihan klien.
                </p>
              </div>
              <ArrowRight className="h-4 w-4 md:h-5 md:w-5 text-slate-400 group-hover:text-slate-600 group-hover:translate-x-1 transition-all shrink-0" />
            </div>
      ...
      <section className="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden">
        <div className="px-4 md:px-6 py-4 border-b border-slate-100">
          ...
          <p className="text-xs md:text-sm text-slate-500 mt-1">
            Ringkasan transaksi arus kas terbaru.
          </p>
        </div>
        ...
        <div className="overflow-x-auto w-full -mx-4 px-4 md:mx-0 md:px-0">
          <table className="w-full min-w-max text-left">
            <thead className="bg-slate-50/80">
              <tr>
                <th className="px-4 md:px-6 py-3 text-[11px] font-bold uppercase tracking-wider text-slate-500">
                  ID Transaksi
                </th>
                <th className="px-4 md:px-6 py-3 text-[11px] font-bold uppercase tracking-wider text-slate-500">
                  Tanggal
                ...
                <th className="px-4 md:px-6 py-3 text-[11px] font-bold uppercase tracking-wider text-slate-500 text-right">
                  Nominal
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {recentTransactions.map((item) => (
                ...
                >
                  <td className="px-4 md:px-6 py-3 text-sm font-mono text-slate-600">
                    {item.id}
                  </td>
                  <td className="px-4 md:px-6 py-3 text-sm text-slate-600">
                    {item.date}
                  </td>
                  <td className="px-4 md:px-6 py-3 text-sm text-slate-800">
                    {item.description}
                  </td>
              ...
              {!isLoading && recentTransactions.length === 0 && (
                <tr>
                  <td
                    colSpan={5}
                    className="px-4 md:px-6 py-6 text-sm text-slate-500"
                  >
