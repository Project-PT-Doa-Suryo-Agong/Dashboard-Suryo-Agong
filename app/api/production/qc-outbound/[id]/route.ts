...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error) return fail(ErrorCode.DB_ERROR, "Gagal update QC outbound.", 500, error.message);
  if (!data) return fail(ErrorCode.NOT_FOUND, "Data QC outbound tidak ditemukan.", 404);
  return ok({ qc_outbound: data });
}
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error, deleted } = await deleteQCOutbound(auth.ctx.supabase, id);
  if (error) return fail(ErrorCode.DB_ERROR, "Gagal hapus QC outbound.", 500, error.message);
  if (!deleted) return fail(ErrorCode.NOT_FOUND, "Data QC outbound tidak ditemukan.", 404);
  return ok(null, "QC outbound berhasil dihapus.");
}
